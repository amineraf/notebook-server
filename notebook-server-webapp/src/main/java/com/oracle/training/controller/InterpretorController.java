...
public class InterpretorController {
    ...
    @PostMapping("/execute")
    public Callable<ScriptResponse> execute(@ValidatePayload @RequestBody ScriptRequest scriptRequest, HttpSession httpSession) throws JsonProcessingException {
        return new Callable<ScriptResponse>() {
            ...
            public ScriptResponse call() {
                ...
                if (!supportedLang(scriptRequest.getLanguage())){
                    return new ScriptResponse("","Language not supported yet!");
                }
